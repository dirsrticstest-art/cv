  ...
  return (
    <section className="space-y-6">
      <div className="flex items-center gap-3 border-b border-gray-800 light:border-gray-200 pb-3">
        ...
        <h2 className="text-2xl font-bold text-white light:text-gray-900">Personal & Academic Profile</h2>
      </div>
      ...
      <div className="grid md:grid-cols-12 gap-8 items-start">
        ...
        <div className="md:col-span-5 bg-gray-900/90 light:bg-gray-50 border border-gray-800 light:border-gray-200 rounded-xl p-6 space-y-4 shadow-sm">
          <div className="flex items-center justify-between border-b border-gray-800 light:border-gray-200 pb-2.5">
            <h3 className="text-xs font-mono text-purple-400 uppercase tracking-wider font-bold">
              Candidate Fact Sheet
            </h3>
            <span className="text-[10px] font-mono text-emerald-400 bg-emerald-950/60 light:bg-emerald-50 px-2 py-0.5 rounded border border-emerald-800/40 light:border-emerald-200">
              Verified Info
            </span>
          </div>
          ...
          <div className="space-y-3 text-xs">
            {personalDetails.map((item, idx) => (
              <div key={idx} className="flex flex-col space-y-0.5">
                <span className="text-gray-400 light:text-gray-500 font-mono text-[11px]">{item.label}:</span>
                <span className="text-gray-200 light:text-gray-700 font-medium">{item.value}</span>
              </div>
            ))}
          </div>
        </div>
